        ...
        try:
            async with websockets.connect(uri) as websocket_exchange:
                ...
                while True:
                    ...
                    obj = json.loads(msg)
                    logger.debug(f"{stream_name} queue length: {len(msg_q)}. data length:{len(obj['data'])}")
                    ...
                    for data in obj['data']:
                        ...
                        await redis_db.xadd(name=stream_name, fields=data, id=new_id, )
                        last_id = new_id
...
def start_trades(background_tasks: BackgroundTasks, stream:str = "publicTrade.ETHUSDT"):

    background_tasks.add_task(fetch_exchange_ws_stream, stream=stream,)# redis_db=redis_db )
